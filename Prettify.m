...
for s=1:c(1)   % for each row
    ...
    m=size(M); 
    for i=1:m(2),                                %write them as x_i
        var = [var, ' x',num2str(sigma(M(i)))];  % with the appropriate index from sigma
    end
    N=find(k==0);  % find the 0's
    n=size(N);
    for j=1:n(2)                                    % write them as (1-x_i)
        var = [var,' (1-x',num2str(sigma(N(j))),')']; % with the appropriate index from sigma
    end
    D=char(D,var);
end
end
